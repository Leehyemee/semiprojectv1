...
import com.example.svsvdvdv.semiprojectv1.repository.GalleryRepository;
...
public class GalleryServiceImpl implements GalleryService{
    ...
    public boolean newGalleryImage(NewGalleryDTO gal, List<MultipartFile> ginames) {
        // 작성한 게시글을 gallerys 테이블에 저장하고, 생성된 글번호를 알아냄
        int gno = -999;
        if (galleryMapper.insertGallery(gal) > 0)
            gno = gal.getGgno();

        // 첨부된 파일을 업로드 처리하고
        // 알아낸 글번호로 첨부된 파일들에 대한 정보를 gallery_images 테이블에 저장

        // 첨부된 파일 들 중 첫번째 이미지 파일을 썸네일 처리
