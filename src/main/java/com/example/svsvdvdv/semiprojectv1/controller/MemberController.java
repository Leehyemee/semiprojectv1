...
import com.example.svsvdvdv.semiprojectv1.jwt.JwtTokenProvider;
...
public class MemberController {
    ...
    // ResponseEntity는 스프링에서 HTTP와 관련된 기능을 구현할 떄 사용
    // 상태코드, HTTP헤더, HTTP본분 등을 명시적으로 설정 가능
    @PostMapping("/join")
    ...
    public ResponseEntity<?> loginok(MemberDTO member, HttpServletResponse res) {
        ...
        try {
            ...
            authenticationManager.authenticate(
                    new UsernamePasswordAuthenticationToken(member.getUserid(), member.getUserpwd())
            );
            ...
            // 인증 실패 시 상태코드 401으로 응답. - 아이디나 비밀번호 잘못 입력 시
